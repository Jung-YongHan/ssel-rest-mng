...
### 4-6. PWA 홈 화면 아이콘·설치 안내 (2026-08-19 **원인 확정·종결**)
...
위 두 가지를 고친 뒤에도 iOS 26.6/27 실기기에서 글자 타일이 재현됐다 (2026-08-19).
그날 로그·검증으로 확정한 사실:

- iOS 는 추가 시트를 여는 순간 모든 아이콘 후보(v3 PNG·apple-touch-icon 4종·favicon
  2종)를 **전부 200 으로 받아갔다** — 그런데도 글자 타일. 후보 전체 순회는 "받은 것을
  하나도 못 쓴다"는 뜻.
- 공개 URL 이 주는 바이트 = 디스크 파일 (sha256 일치), 헤더 정상, Let's Encrypt 유효,
  공인 DNS 정상. **서버 측 원인은 소거 완료.**
- Safari 와 iOS Chrome 모두 실패 (웹클립은 공용 시스템 서비스) ↔ 같은 기기에서
  uihy.space 는 정상 → iOS 쪽에서 이 도메인/선언 모양에 걸리는 무언가가 남아 있다.

**최종 원인 (실기기 A/B 로 확정)**: 마크업·파일·서버가 아니라 **외부(LTE) 인입
경로의 TLS**. 교내망(Wi-Fi)에서는 정상 인증서가 보이고 홈 화면 추가 시 아이콘이
정상 반영되지만, LTE 로는 Safari 가 "보안 안 됨"을 표시했다(외부 인입 443 이
캠퍼스 방화벽 등에서 다른 인증서를 제시하는 것으로 추정). iOS 웹클립 파이프라인은
신뢰 안 되는 연결에서 받은 아이콘을 조용히 버리고 글자 타일로 폴백한다 —
서버 로그상 아이콘을 전부 200 으로 받아가고도 실패했던 이유.

- 당장의 안내: 홈 화면 추가는 **교내망에서** 할 것 (추가 후 LTE 사용은 정상)
- 남은 일: 전산 부서에 예외 요청. **외부 관측 증거 (SSL Labs, 2026-08-19)**:
  외부에서 `210.117.175.116:443` 에 SNI `ssel.qrade.co.kr` 로 접속하면 우리
  Let's Encrypt 인증서가 아니라 **CN=jbnu.ac.kr (Sectigo DV)** 인증서가 제시됨
  (등급 T = 호스트명 불일치, SSL3.0/TLS1.0/1.1 활성). 캠퍼스 경계 장비가 TLS 를
  종단한 뒤 중계하는 구성 — 요청 사항: 외부 인입 443 이 `ssel.qrade.co.kr`
  (SNI 기준) 에 대해서는 TLS 를 건드리지 않고 통과(passthrough)하도록 예외 처리
- 기기 판정 페이지 `/secure-check.html` 을 남겨 두었다 (외부 경로가 고쳐졌는지
  LTE 에서 isSecureContext/보안 표시로 확인). 해결되면 삭제할 것

**같은 원인의 두 번째 증상 — 외부(LTE)에서 간헐적으로 로그인 실패** (2026-08-19 관측).
증상은 "화면은 뜨는데 로그인만 안 됨"이고, 잠시 뒤 저절로 복구된다. 판정 근거:

- 실패 중에도 서버는 정상이었다 — 내부·ingress·외부 관측점 모두 `/api/health` 200,
  로그인 엔드포인트 401 정상.
- **실패한 요청이 서버·인그레스 로그에 아예 없었다.** 앱 버그·서버 오류라면 요청은
  도착하고 4xx/5xx 가 남는다. 기록이 없다는 것은 기기~서버 사이에서 끊겼다는 뜻.
- 화면이 뜨는 이유는 앱 껍데기가 서비스워커 캐시에서 나오기 때문이다. 그래서 서버로
  나가야 하는 로그인 요청만 실패해 "로그인 문제"로 보인다.
- 메커니즘: 위 인증서 가로채기 + 우리가 보내는 HSTS(`max-age=31536000`). HSTS 가 걸린
  사이트에서 인증서가 신뢰되지 않으면 Safari 는 "계속" 우회 없이 **연결을 즉시 차단**한다
  — 서버에 아무 기록이 남지 않는 이유. 경계 장비를 타는지에 따라 갑자기 막히고 풀린다.
- **HSTS 는 빼지 말 것.** 빼면 중간에서 복호화하는 인증서를 사용자가 눌러 넘어가게 된다.
- 재발 시 판정: LTE 에서 `/secure-check.html` 의 "서버 연결 확인" 항목만 본다.
  `요청 실패: ...` → 네트워크 경로 확정(위 예외 요청 필요) / `HTTP 200` → 기기 캐시
  문제이므로 같은 화면의 '서비스워커 해제 + 캐시 삭제' 버튼으로 복구한다.

...
### 4-6. PWA 홈 화면 아이콘·설치 안내 (2026-08-19 현재 **미해결, 실험 진행 중**)
...
진행 중인 실험: 정상 동작 사이트와 아이콘 선언 모양을 동일하게 복제 —
단일 apple-touch-icon(sizes 없음)·rel=icon 제거·manifest 아이콘 2개(192/512,
상대 경로, purpose 'any maskable' 결합)·-v4 URL(캐시 오염 변수 제거).
이것도 실패하면 다음은 임시 서브도메인으로 도메인 키 오염 여부 판정.
